...
class VideoClipMoeCrossAttentionModel(nn.Module):
    ...
    def build(self,):
        self.clipmodel = CLIPModel.from_pretrained(self.clip_model_name, config=self.clip_config)
    ...
    def forward(self, sample_list):
        ...
        generate_captions_input_ids = generate_captions.input_ids.reshape(b*number_of_captions, sequence_length)
        generate_captions_input_mask = generate_captions.input_mask.reshape(b*number_of_captions, sequence_length)
        generate_caption_features = self.clipmodel.get_text_features(generate_captions_input_ids, generate_captions_input_mask)
        generate_caption_features = generate_caption_features.reshape(b, number_of_captions, -1)
        ...
        text_features = self.clipmodel.get_text_features(caption.input_ids, caption.input_mask)
        
        video_features_concat = torch.cat([generate_caption_features, vis_features], dim=1)
        video_features = self.moe_layer({"caption": generate_caption_features, "video": vis_features})
